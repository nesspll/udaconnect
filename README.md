...
**get the name of PostgresDB Pod**

`kubectl get pods`

**Run the Script and pass the POD name**

`run_db_command.sh POD_NAME`
<hr>

Deploy a Kafka Client Producer for TESTING kafka Consumer if you will:

`kubectl run my-release-kafka-client --restart=Never --image docker.io/bitnami/kafka:2.8.0-debian-10-r30 --namespace default --command -- sleep infinity
`
...
**Now you have a Producer ready to send messages, send this json payload below and refresh the page:**

`{'first_name':'John','last_name':'Doe','company_name':'PlaySmar Inc.'}`

You should see that person on the front end page.

<hr>

Import POSTMAN collection to POSTMAN
- You can play around with sending and retrieving data from the apis.
...
## Part 1: Monolith version (Starter)
## Overview
### Background
